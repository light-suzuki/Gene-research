...
export const SequenceViewer: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const msg =
        e instanceof Error ? e.message : "解析中に思わぬエラーが起きました。";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section>
      <h2 className="panel-title">Sequence 解析 & SeqViz</h2>
      <p className="panel-hint">
        FASTA または DNA 配列を貼り付けて、ORF・制限酵素・SeqViz の両ビューに反映します。
      </p>
        ...
        <div className="seq-results">
          {basic && (
            <section className="seq-result-block">
              <h2>Basic 情報</h2>
              <p>塩基数: {basic.length} bp</p>
              <p>GC 含量: {basic.gc_percent.toFixed(2)} %</p>
