...
export const PrimerReversePanel: React.FC = () => {
  ...
  const needsqueryTorefXlsxMap = useMemo(() => {
    ...
    const isqueryGene = (g?: string | null) => !!g && /^Psat\\.cameor\\.query\\./i.test(g);
                ...
                <table className="seq-table primer-bulk-table">
                  ...
                  <tbody>
                    {amplicons.map((a, idx) => {
                      ...
                      const queryGeneCandidates = uniqueGenes.filter((g) => /^Psat\\.cameor\\.query\\./i.test(g));
                      ...
                      const txGeneName =
                        firstGene && /^KIW[0-9A-Za-z_]+/.test(firstGene)
                          ? uniqueGenes.find((g) => /^Psat\\d+G\\d+/i.test(g)) ??
                          uniqueGenes.find((g) => /^Psat/i.test(g)) ??
                          null
                          : null;
                    ...
                    {bulkResults.map((r) => {
                      ...
                      const coordGene = (d?: (typeof r.perDb)[number]) => {
                        ...
                        const parts = d.topAmplicons.map((a, idx) => {
                          ...
                          const dstGene = dstGenes[0] ?? null;
                          const xlsxMapping =
                            wantsqueryTorefView && isqueryDb && genequery && queryTorefXlsxMap && /^Psat\\.cameor\\.query\\./i.test(genequery)
