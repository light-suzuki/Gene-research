...
export const BlastPanel: React.FC = () => {
  ...
  const needsqueryTorefXlsxMap = useMemo(() => {
    ...
    return hits.some((h) => {
      ...
      return genes.some((g) => /^Psat\\.cameor\\.query\\./i.test(g));
    });
  ...
  const renderHitRow = (hit: BlastHit, index: number) => {
    ...
    const queryGeneCandidates = uniqGenes.filter((g) => /^Psat\\.cameor\\.query\\./i.test(g));
    ...
    const kiwGenes = uniqGenes.filter((g) => /^KIW[0-9A-Za-z_]+/.test(g));
    const psatGenes = uniqGenes.filter((g) => /^Psat[0-9A-Za-z]+/i.test(g));
    const genes =
      kiwGenes.length > 0
        ? kiwGenes
        : psatGenes.length > 0
    ...
    const txGeneName =
      txGeneId && /^KIW[0-9A-Za-z_]+/.test(txGeneId) ? (psatGenes[0] ?? null) : null;
                        ...
                        <>
                          ...
                          <button
                            ...
                            disabled={
                              ...
                              `${(inferEnsemblPlantsSpecies({ geneId: dstGene, dbLabel: "UserDB_ref" }) || "user_species").trim()}|${dstTranscriptId}`
                            }
                            onClick={() => {
                              const sp =
                                inferEnsemblPlantsSpecies({ geneId: dstGene, dbLabel: "UserDB_ref" }) ||
                      ...
                      {xlsxTxExportUrl && xlsxTranscriptId ? (
                        <>
                          ...
                          <button
                            ...
                            disabled={
                              ensemblExportBusyKey ===
                              `${(inferEnsemblPlantsSpecies({ geneId: xlsxV1Gene, dbLabel: "UserDB_ref" }) || "user_species").trim()}|${xlsxTranscriptId}`
                            }
                            onClick={() => {
                              const sp =
                                inferEnsemblPlantsSpecies({ geneId: xlsxV1Gene, dbLabel: "UserDB_ref" }) ||
    ...
    <section className="seq-result-block">
      ...
      <div className="blast-grid">
        <div className="blast-controls">
          ...
          <details className="ui-details">
            ...
            <div className="ui-details-body">
              ...
              {false && useEnsembl && (
                <label className="seq-label">
                  Ensembl species（例: homo_sapiens, user_species）:
                  <input
                    type="text"
                    className="seq-input"
                    value={ensemblSpecies}
                    onChange={(e) => setEnsemblSpecies(e.target.value)}
                  />
                </label>
              )}
              ...
              {false && useNcbi && (
                <div className="primer-row">
                  ...
                  <label className="seq-label">
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                      <div style={{ display: "flex", gap: "0.4rem" }}>
                        <input
                          ...
                          disabled={useNcbiPresetTargets}
                        />
                        <button
                          type="button"
                          className="link-button"
                          onClick={() => toggleFavoriteNcbiQuery(ncbiQuery)}
                          disabled={!ncbiQuery.trim()}
                          title={
                      ...
                      {(recentNcbiQueries.length > 0 || favoriteNcbiQueries.length > 0) && (
                        <div style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
                          {favoriteNcbiQueries.length > 0 && (
                            <div style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
                              <span className="seq-hint">お気に入り:</span>
                              <select
                                className="seq-input"
                          ...
                          {recentNcbiQueries.length > 0 && (
                            <div style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
                              <span className="seq-hint">最近使ったクエリ:</span>
                              <select
                                className="seq-input"
                                style={{ maxWidth: "260px" }}
                                value=""
                    ...
                    <span className="seq-hint">
                      プリセットON時は設定ファイルの公開プリセットを使用します。
                    </span>
                  </label>
                  <label className="seq-label">
                    プリセット種で並列実行:
                    <div className="blast-backend-row">
                      <label>
                        <input
                          type="checkbox"
                          checked={useNcbiPresetTargets}
                          onChange={(e) => setUseNcbiPresetTargets(e.target.checked)}
                        />{" "}
                        公開プリセットを並列で実行
                      </label>
                      {useNcbiPresetTargets && (
                        <div className="ncbi-preset-row">
                          <label>
                            <input
                              type="checkbox"
                              checked={useArabidopsis}
                              onChange={(e) => setUseArabidopsis(e.target.checked)}
                            />{" "}
        ...
        <div className="blast-results">
          ...
          {false && resultTab === "ncbi" && (
            <div className="blast-two-col">
              <div className="blast-table-block" style={{ flex: 1 }}>
                <h3 className="blast-table-title">NCBI ヒット</h3>
                {ncbiSortedHits.length === 0 ? (
                  <p className="seq-hint">ヒットなし</p>
                ) : (
                  <div className="table-scroll">
                    <table className="seq-table">
                      ...
                      <tbody>{ncbiSortedHits.slice(0, MAX_DISPLAY_HITS).map(renderHitRow)}</tbody>
                    </table>
                  </div>
                )}
              </div>
