...
const buildMarkdown = (res: CapsDesignResponse): string => {
  ...
  res.markers.slice(0, 200).forEach((m) => {
    ...
    // gene_label alone might be Psat05G... which is Ensembl.
    // If it's something else, maybe Navigator.
    // For safety in Markdown, let's stick to ensemblGeneUrl for Psat IDs and maybe nothing for others unless we want to embed localhost links which might not work for sharing.
          ...
          {m.gene_label ? (
            <EnsemblLinksInline
              ...
              /* We don't have the specific DB for this gene label easily here, but usually it matches the ref/alt DBs or blast DBs.
                 ...
                 If ID is Psat... it goes to Ensembl.
                 ...
                 If Ref DB is "fra_v1" (local only), it will link to Navigator.
